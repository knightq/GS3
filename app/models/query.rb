...
#  serialize :column_names
  ...
  attr_accessor :prodotto, :utente, :filters, :column_names,:cda_prodotto, :user_id
 
  #attr_protected :cda_prodotto, :user_id
...
#    set_language_if_valid(User.current.language)
  ...
  def editable_by?(user)
    ...
    is_public && !@is_for_all && user.allowed_to?(:manage_public_queries, prodotto)
  end
...
#                           "done_ratio" =>  { :type => :integer, :order => 14 }
    ...
    if prodotto
      ...
      unless @prodotto.issue_categories.empty?
        @available_filters["category_id"] = { :type => :list_optional, :order => 6, :values => @prodotto.issue_categories.collect{|s| [s.name, s.id.to_s] } }
      end
      unless @prodotto.versions.empty?
        @available_filters["fixed_version_id"] = { :type => :list_optional, :order => 7, :values => @prodotto.versions.sort.collect{|s| [s.name, s.id.to_s] } }
      end
      unless @prodotto.active_children.empty?
        @available_filters["subproject_id"] = { :type => :list_subprojects, :order => 13, :values => @prodotto.active_children.collect{|s| [s.name, s.id.to_s] } }
      end
      add_custom_fields_filters(@prodotto.all_issue_custom_fields)
    else
  ...
  def project_statement
    ...
    if prodotto && !@prodotto.active_children.empty?
      ...
      project_clauses << "#{Project.table_name}.id = %d" % prodotto.id
    end
    project_clauses <<  Prodotto.allowed_to_condition(User.current, :view_issues)
